...
# run mhcflurry
call(["mhcflurry-predict mhcflurry_input.csv --out predictions.csv > temp_out.txt"], shell=True)
# extract predicted labels
call(["grep \"" + hla_name + "\" predictions.csv | awk -F \"\\\"*,\\\"*\" '{ print $3 }' > out1.txt"], shell=True)
...
call(["mhcflurry-predict mhcflurry_input.csv --out predictions.csv > temp_out.txt"], shell=True)
# extract predicted labels
call(["grep \"" + hla_name + "\" predictions.csv | awk -F \"\\\"*,\\\"*\" '{ print $3 }' > out0.txt"], shell=True)
...
f = open("out1.txt", 'r')
for line in f:
	transformed_val = np.log(float(line)) / np.log(50000)
	y_scores_pos.append(1 - transformed_val)
	#y_true.append(1)
f.close()
f = open("out0.txt", 'r')
for line in f:
	transformed_val = np.log(float(line)) / np.log(50000)
	y_scores_neg.append(1 - transformed_val)
f.close()
...
f = open("alleleoutput.csv", 'a+')
for i,v in enumerate(output_vars):
    if i == len(output_vars) - 1: f.write(str(v) + "\n")
    else: f.write(str(v) + ",")
f.close()

#print(allele+","+str(acc)+","+str(auroc)+","+str(auprc)+","+str(pp1))

call(["rm temp.txt temp_out.txt mhcflurry_input.csv predictions.csv out0.txt out1.txt"], shell=True)
