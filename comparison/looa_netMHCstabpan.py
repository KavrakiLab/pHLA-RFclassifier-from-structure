...
# run netMHCstabpan
call(["~/Downloads/netMHCstabpan-1.0/netMHCstabpan -p temp.txt -a " + hla_name_netMHC + " > temp_out.txt"], shell=True)
# extract predicted labels
call(["grep \"PEPLIST \" temp_out.txt | awk '{ print $5 }' > out1.txt"], shell=True)
...
call(["~/Downloads/netMHCstabpan-1.0/netMHCstabpan -p temp.txt -a " + hla_name_netMHC + " > temp_out.txt"], shell=True)
# extract predicted labels
call(["grep \"PEPLIST \" temp_out.txt | awk '{ print $5 }' > out0.txt"], shell=True)
...
f = open("out1.txt", 'r')
for line in f:
	y_scores_pos.append(float(line))
	#y_true.append(1)
f.close()
f = open("out0.txt", 'r')
for line in f:
	y_scores_neg.append(float(line))
	#y_true.append(0)
f.close()
